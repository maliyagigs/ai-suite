...
import { Sparkles, Mail, Lock, User as UserIcon, Eye, EyeOff, ShieldCheck, ArrowRight, Chrome, AlertCircle, RefreshCw, CheckCircle2 } from 'lucide-react';
...
export function AuthScreen() {
  ...
  const [errorMsg, setErrorMsg] = useState('');
   ...
   useEffect(() => {
     // Check if Google GIS client script is already in document
     ...
     script.onerror = () => console.warn('Could not load Google GSI client library dynamically.');
     document.body.appendChild(script);

     return () => {
       // Keep script available across auth switches
     };
   }, []);
   ...
   useEffect(() => {
     ...
     // Fetch user configured VITE_GOOGLE_CLIENT_ID or standard app client ID
     const clientId = (import.meta as any).env.VITE_GOOGLE_CLIENT_ID || '629111524631-3q4s91g3c69vtqmok0tu1a1io9haonfl.apps.googleusercontent.com';
     ...
     try {
       ...
       if (google) {
         google.accounts.id.initialize({
           ...
           auto_select: (typeof window !== 'undefined' && window.self === window.top), use_fedcm_for_prompt: (typeof window !== 'undefined' && window.self === window.top), // Auto-select and FedCM disabled in nested/iframe controls to prevent NotAllowedError
         });
         ...
         // Prompt standard Google One Tap automatically for device account syncing
         if (typeof window !== 'undefined' && window.self === window.top) google.accounts.id.prompt((notification: any) => {
           console.log('One Tap active notification:', notification);
         });
       }
     } catch (err) {
   ...
   const [oAuthStage, setOAuthStage] = useState<'picker' | 'consent' | 'manual' | 'processing' | 'done'>('picker');
 
   const GOOGLE_ACCOUNTS = [
     { name: 'Maliyagigs Admin', email: 'maliyagigs@gmail.com', avatar: 'M', desc: 'Platform Owner' },
     { name: 'David Chen', email: 'buyer@melagent.com', avatar: 'D', desc: 'Standard Client' },
     { name: 'Liam Peterson', email: 'liam.peterson@gmail.com', avatar: 'L', desc: 'Verified Buyer' },
     { name: 'Sarah Jenkins', email: 'sarah.jenkins@gmail.com', avatar: 'S', desc: 'Creative Professional' },
   ];
   ...
   const handleSubmit = (e: React.FormEvent) => {
     ...
     if (password.length < 5) {
       setErrorMsg('Password must be at least 5 characters for security verification.');
       return;
     }
 
     // Check specific credentials requested by user
     if (cleanEmail === 'maliyagigs@gmail.com' && password !== 'g2jabB80') {
       setErrorMsg('Incorrect security password for this administrator account.');
       return;
     }
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950 md:p-8 transition-colors duration-200">
      ...
      >
        ...
        <div ref={containerRef} className="w-full max-w-[320px] mx-auto flex flex-col items-center">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {activeTab === 'login' ? 'Authenticate Access' : 'Create & Access Profile'}
        ...
        <div className="flex flex-col items-center gap-3.5 w-full">
          {/* Real Google Standard Button */}
          <div className="w-full flex justify-center items-center">
            <div id="google-gsi-button" className="min-h-[40px] flex justify-center py-1" style={{ width: `${btnWidth}px` }}></div>
          </div>

          <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1 text-center font-medium max-w-[320px]">
            Standard Google device accounts synced
          </p>
          ...
          <button
            ...
            className="text-[10px] text-indigo-500/80 hover:text-indigo-500 transition underline tracking-wide font-sans mt-1"
          >
            Developer Sandbox / Simulate Login
          </button>
        </div>
      ...
      <AnimatePresence>
        {oauthProvider && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={() => setOauthProvider(null)} />
            ...
            >
                ...
                <div>
                  <div className="text-center mb-5">
                    ...
                    <p className="text-xs text-slate-500 mt-1">
                      to continue to <span className="font-bold text-indigo-600 dark:text-indigo-400">MelAgent</span>
                    </p>
                  </div>
                    ...
                    {GOOGLE_ACCOUNTS.map((acc) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-bold text-slate-800 dark:text-slate-100 truncate">{acc.name}</p>
                          <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate">{acc.email}</p>
                        </div>
                  ...
                  <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-slate-100 dark:border-slate-805 space-y-2">
                    <div className="flex items-center gap-2 mb-1.5 pb-1.5 border-b border-slate-200/50 dark:border-slate-800/50">
                      ...
                      <div className="min-w-0">
                        <span className="text-xs font-bold text-slate-800 dark:text-slate-100 block truncate">{oAuthName}</span>
                        <span className="text-[10px] text-slate-550 dark:text-slate-400 truncate block">{oAuthEmail}</span>
                      </div>
                    </div>
                    <p className="text-[11px] text-slate-600 dark:text-slate-350 leading-relaxed">
                      <strong>MelAgent</strong> wants access to your basic developer profile information:
                    </p>
              ...
              {oAuthStage === 'processing' && (
                <div className="text-center py-8 flex flex-col items-center justify-center gap-3">
                  ...
                  <div>
                    <span className="text-xs text-slate-800 dark:text-slate-100 font-bold block">Negotiating OAuth 2.0 handshake...</span>
                    <span className="text-[10px] text-slate-400 mt-1 block">Validating response payloads and redirect keys</span>
                  </div>
                </div>
              )}
              ...
              {oAuthStage === 'done' && (
                <div className="text-center py-8 flex flex-col items-center justify-center gap-3 animate-fade-in">
                  ...
                  <div>
                    <span className="text-xs text-emerald-600 dark:text-emerald-400 font-bold block">Authorization Confirmed!</span>
                    <span className="text-[10px] text-slate-400 mt-1 block">Redirecting to default buyer layout...</span>
                  </div>
                </div>
              )}
