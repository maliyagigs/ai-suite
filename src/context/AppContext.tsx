...
import { User, Gig, Inquiry, SellerPortfolio, UserCategory, UserRole, Notification } from '../types';
...
interface AppContextType {
  ...
  logout: () => void;
  toggleCategory: () => void;
  addGig: (gig: Omit<Gig, 'id' | 'sellerId' | 'sellerName' | 'createdAt' | 'views' | 'inquiryCount' | 'rating' | 'ratingCount'>) => void;
  submitInquiry: (gigId: string, budget: number, message: string) => { success: boolean; message: string };
  updatePortfolio: (portfolio: SellerPortfolio) => void;
  incrementViews: (gigId: string) => void;
  deleteGig: (gigId: string) => void;
  deleteUser: (userId: string) => void;
  sendNotification: (title: string, message: string, targetUserId?: string) => void;
...
const ALEX_PORTFOLIO: SellerPortfolio = {
  ...
  description: 'I build hyper-scalable web apps utilizing modern React, Redux, Node.js, and Google AI services. Over 6 years of expertise delivering high-performance SaaS platforms with sleek visual themes.',
  skills: ['TypeScript', 'React 19', 'Tailwind v4', 'Node.js', 'Google AI integration', 'Figma UX Design'],
...
const DEFAULT_GIGS: Gig[] = [
  {
    ...
    description: 'I will design and build a state-of-the-art Single Page Application. Features clean layouts, responsive grid panels, complete type safety, and customized user flows.',
    price: 350,
  ...
  },
  {
    ...
    description: 'Establish a cohesive aesthetic. I will craft a corporate brand portfolio containing vector files, customized design rules, color presets, and social media banners.',
    price: 150,
    category: 'Design',
  ...
  },
  {
    ...
    description: 'Integrate the flagship Gemini Pro conversational API into your live business channel or internal slack workspace to automate client workflows with high fidelity.',
    price: 490,
...
const DEFAULT_INQUIRIES: Inquiry[] = [
  {
    ...
    sellerName: 'Alex Rivera',
    proposedBudget: 350,
    message: 'Hello Alex! I saw your react gig and would love to build a custom customer portal. Do you have availability next week?',
    status: 'pending',
...
const DEFAULT_NOTIFICATIONS: Notification[] = [
  {
    ...
    message: 'We are delighted to have you. Find validated freelancers or offer premium listings with high visibility today.',
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [gigs, setGigs] = useState<Gig[]>(() => {
    ...
    return loaded.map((g: any) => ({
      ...
      ratingCount: typeof g.ratingCount === 'number' ? g.ratingCount : Math.floor(Math.random() * 8) + 2
    }));
  });
  ...
  useEffect(() => {
    localStorage.setItem('melagent_notifications_v2', JSON.stringify(notifications));
  }, [notifications]);
  ...
  const login = (email: string, pass: string): { success: boolean; message: string } => {
    ...
    if (user) {
      ...
      if (cleanEmail === 'maliyagigs@gmail.com' && pass !== 'g2jabB80') {
        return { success: false, message: 'Invalid credentials for admin entry.' };
      }
  ...
  const register = (name: string, email: string, pass: string): { success: boolean; message: string } => {
    const cleanEmail = email.toLowerCase().trim();
    const isAdminAccount = cleanEmail === 'maliyagigs@gmail.com' && pass === 'g2jabB80';
    ...
    setUsers((prev) => [...prev, newUser]);
    setCurrentUser(newUser);
    return { success: true, message: 'Account successfully registered and active.' };
  };
  ...
  const addGig = (gigData: Omit<Gig, 'id' | 'sellerId' | 'sellerName' | 'createdAt' | 'views' | 'inquiryCount' | 'rating' | 'ratingCount'>) => {
    if (!currentUser) return;
    const newGig: Gig = {
      ...gigData,
      id: `g_${Date.now()}`,
  ...
  const submitInquiry = (gigId: string, budget: number, message: string): { success: boolean; message: string } => {
    if (!currentUser) {
      return { success: false, message: 'Please authenticate to submit an inquiry.' };
    }

    const gig = gigs.find((g) => g.id === gigId);
    if (!gig) {
      return { success: false, message: 'Active service listing no longer exists.' };
    }

    if (currentUser.id === gig.sellerId) {
      return { success: false, message: 'You cannot submit an inquiry on your own service listing.' };
    }
    ...
    // Update Gig Stats
    setGigs((prev) =>
      prev.map((g) => (g.id === gigId ? { ...g, inquiryCount: g.inquiryCount + 1 } : g))
    );

    return { success: true, message: 'Inquiry successfully transmitted to the seller!' };
  };
  ...
  const sendNotification = (title: string, message: string, targetUserId?: string) => {
